...
**Estado:** APROBADA
**Tarea anterior:** AUTH-UI-059 — Registrar problemas encontrados
**Tarea siguiente:** OPS-REC-001 — Definir el contrato canónico de recetas y acceso contextual
**Tipo de tarea:** puerta documental integral; contrato y matriz individual para decidir aprobación, bloqueo o no aplicabilidad de cada superficie antes de retirarla del roadmap
**Bloque:** BLOQUE I — Protección y estados de interfaz
**Repositorio propietario:** `vento-group-sas/vento-shell`
**Archivo propietario:** `docs/plan-canonico/modular/bloques/I_NAVEGACION_Y_PANTALLAS/06_EXPERIENCIA_USABILIDAD_Y_APROBACION.md`
**Estado físico resultante:** `NO_PHYSICAL_INSTANCE`
**Cambios físicos autorizados:** ninguno; la puerta registra decisiones y bloqueos pero no modifica código, permisos, datos, Supabase, infraestructura ni despliegues
**Requisitos de prueba creados o modificados:** 0
...
Definir la puerta final de experiencia del minibloque `AUTH-UI-052..060` y reconciliar exactamente una vez las 177 superficies canónicas antes de que cualquiera pueda considerarse aprobada y retirada del roadmap de trabajo pendiente.

La aprobación documental de este contrato no inventa aprobaciones operativas:

```text
CONTRATO AUTH-UI-060 APROBADO
!=
177 PANTALLAS APROBADAS

RESULTADO ACTUAL SIN EVIDENCIA REAL
=
BLOQUEADA

APROBADA
=
SOLO DESPUES DE EVIDENCIA + RECONCILIACION + REVIEW REAL
```

El estado `BLOQUEADA` es la decisión segura mientras faltan pruebas, reconciliación de hallazgos, implementación requerida, revisor o fecha.

---

#### 2. Continuidad vigente corregida

La referencia histórica del marcador original indicaba `AUTH-UI-061` como sucesora. La ruta normal vigente ubica `AUTH-UI-052..060` dentro de `PHASE-05-NEXO` y realiza handoff después de 060 hacia:

```text
OPS-REC-001 — Definir el contrato canónico de recetas y acceso contextual
```

`AUTH-UI-061` pertenece a `PHASE-02-VISO-SCHEDULE-DELTA`, actualmente diferida, por lo que no se usa como continuidad normal de este cierre.
...
- `AUTH-UI-057` y sus criterios por superficie;
- `AUTH-UI-058-USER-TEST-GATE-001`;
- las sesiones y evidencia reales producidas por 058;
- `AUTH-UI-059` y su reconciliación de observaciones/hallazgos;
- problemas vinculados, severidades, resoluciones, diferimientos y retests;
- estado de implementación necesario para cada superficie;
- propiedad funcional de la aplicación primaria;
- propiedad técnica del repositorio de la aplicación.

La versión completa aprobada usada como base inmediata para esta redacción es `AUTH-UI-059`.

---

#### 4. Propiedad usada por la matriz

La aplicación primaria conserva la responsabilidad funcional de evolución, retiro y trazabilidad de la superficie. El repositorio técnico correspondiente se registra como propietario técnico de referencia.

| `app_code` | Propietario funcional | Propietario técnico |
| --- | --- | --- |
| `shell` | `shell` | `vento-shell` |
| `anima` | `anima` | `vento-anima` |
| `viso` | `viso` | `vento-viso` |
| `nexo` | `nexo` | `vento-nexo` |
| `fogo` | `fogo` | `vento-fogo` |
| `origo` | `origo` | `vento-origo` |
| `pulso` | `pulso` | `vento-pulso` |
| `numera` | `numera` | `vento-numera` |
| `pass` | `pass` | `vento-pass` |

Esta asociación no transfiere propiedad de datos ni autorización entre aplicaciones.

---

#### 5. Resultados admisibles

| Resultado | Condición |
| --- | --- |
| `APROBADA` | evidencia real suficiente; cero bloqueadores incompatibles; autorización/accesibilidad/recuperación conformes; implementación necesaria disponible; revisor y fecha registrados |
| `BLOQUEADA` | falta evidencia, existe problema crítico/alto sin resolución aceptada, incumplimiento de autorización/accesibilidad/recuperación, implementación pendiente o bloqueo sin cerrar |
| `NO_APLICA` | la superficie no aplica al alcance evaluado y existe justificación revisable |

No existe un cuarto resultado operativo. La ausencia de evidencia nunca se transforma en aprobación tácita.

---

#### 6. Matriz final de 177 superficies

La matriz siguiente contiene las 177 identidades exactamente una vez. Su estado actual es `BLOQUEADA` porque las sesiones, reconciliación final de problemas, revisor y fecha todavía no han sido ejecutados como evidencia real.

| Aplicación | Pantalla | Propietario funcional | Propietario técnico | Actores probados | Dispositivos probados | Criterios consumidos | Sesiones de evidencia | Problemas vinculados | Bloqueadores abiertos | Resultado actual | Revisor | Fecha | Destino de bloqueo |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `shell` | `VSCREEN-0001` | `shell` | `vento-shell` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-ENTRY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `shell` | `VSCREEN-0002` | `shell` | `vento-shell` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONTEXTUAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `shell` | `VSCREEN-0003` | `shell` | `vento-shell` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONTEXTUAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `shell` | `VSCREEN-0004` | `shell` | `vento-shell` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONTEXTUAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `shell` | `VSCREEN-0005` | `shell` | `vento-shell` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `shell` | `VSCREEN-0006` | `shell` | `vento-shell` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `viso` | `VSCREEN-0007` | `viso` | `vento-viso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-ENTRY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `viso` | `VSCREEN-0008` | `viso` | `vento-viso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `viso` | `VSCREEN-0009` | `viso` | `vento-viso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `viso` | `VSCREEN-0010` | `viso` | `vento-viso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `viso` | `VSCREEN-0011` | `viso` | `vento-viso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `viso` | `VSCREEN-0012` | `viso` | `vento-viso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONDITIONAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `viso` | `VSCREEN-0013` | `viso` | `vento-viso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `viso` | `VSCREEN-0014` | `viso` | `vento-viso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `viso` | `VSCREEN-0015` | `viso` | `vento-viso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `viso` | `VSCREEN-0016` | `viso` | `vento-viso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONDITIONAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `viso` | `VSCREEN-0017` | `viso` | `vento-viso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONDITIONAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `viso` | `VSCREEN-0018` | `viso` | `vento-viso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONDITIONAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `viso` | `VSCREEN-0019` | `viso` | `vento-viso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `viso` | `VSCREEN-0020` | `viso` | `vento-viso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `viso` | `VSCREEN-0021` | `viso` | `vento-viso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `viso` | `VSCREEN-0022` | `viso` | `vento-viso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `viso` | `VSCREEN-0023` | `viso` | `vento-viso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONDITIONAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `viso` | `VSCREEN-0024` | `viso` | `vento-viso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONDITIONAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `viso` | `VSCREEN-0025` | `viso` | `vento-viso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `viso` | `VSCREEN-0026` | `viso` | `vento-viso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `anima` | `VSCREEN-0027` | `anima` | `vento-anima` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-ENTRY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `anima` | `VSCREEN-0028` | `anima` | `vento-anima` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `anima` | `VSCREEN-0029` | `anima` | `vento-anima` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `anima` | `VSCREEN-0030` | `anima` | `vento-anima` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONDITIONAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `anima` | `VSCREEN-0031` | `anima` | `vento-anima` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONDITIONAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `anima` | `VSCREEN-0032` | `anima` | `vento-anima` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `nexo` | `VSCREEN-0033` | `nexo` | `vento-nexo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-ENTRY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `nexo` | `VSCREEN-0034` | `nexo` | `vento-nexo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `nexo` | `VSCREEN-0035` | `nexo` | `vento-nexo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `nexo` | `VSCREEN-0036` | `nexo` | `vento-nexo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `nexo` | `VSCREEN-0037` | `nexo` | `vento-nexo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `nexo` | `VSCREEN-0038` | `nexo` | `vento-nexo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `nexo` | `VSCREEN-0039` | `nexo` | `vento-nexo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `nexo` | `VSCREEN-0040` | `nexo` | `vento-nexo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `nexo` | `VSCREEN-0041` | `nexo` | `vento-nexo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONDITIONAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `nexo` | `VSCREEN-0042` | `nexo` | `vento-nexo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONDITIONAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `nexo` | `VSCREEN-0043` | `nexo` | `vento-nexo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `nexo` | `VSCREEN-0044` | `nexo` | `vento-nexo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `nexo` | `VSCREEN-0045` | `nexo` | `vento-nexo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `nexo` | `VSCREEN-0046` | `nexo` | `vento-nexo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `nexo` | `VSCREEN-0047` | `nexo` | `vento-nexo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `nexo` | `VSCREEN-0048` | `nexo` | `vento-nexo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `nexo` | `VSCREEN-0049` | `nexo` | `vento-nexo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `nexo` | `VSCREEN-0050` | `nexo` | `vento-nexo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `nexo` | `VSCREEN-0051` | `nexo` | `vento-nexo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONDITIONAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `nexo` | `VSCREEN-0052` | `nexo` | `vento-nexo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `nexo` | `VSCREEN-0053` | `nexo` | `vento-nexo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `nexo` | `VSCREEN-0054` | `nexo` | `vento-nexo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `fogo` | `VSCREEN-0055` | `fogo` | `vento-fogo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-ENTRY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `fogo` | `VSCREEN-0056` | `fogo` | `vento-fogo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `fogo` | `VSCREEN-0057` | `fogo` | `vento-fogo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `fogo` | `VSCREEN-0058` | `fogo` | `vento-fogo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `fogo` | `VSCREEN-0059` | `fogo` | `vento-fogo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `fogo` | `VSCREEN-0060` | `fogo` | `vento-fogo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONDITIONAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `fogo` | `VSCREEN-0061` | `fogo` | `vento-fogo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `fogo` | `VSCREEN-0062` | `fogo` | `vento-fogo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `fogo` | `VSCREEN-0063` | `fogo` | `vento-fogo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONDITIONAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `fogo` | `VSCREEN-0064` | `fogo` | `vento-fogo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `fogo` | `VSCREEN-0065` | `fogo` | `vento-fogo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `fogo` | `VSCREEN-0066` | `fogo` | `vento-fogo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `fogo` | `VSCREEN-0067` | `fogo` | `vento-fogo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONDITIONAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `origo` | `VSCREEN-0068` | `origo` | `vento-origo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `origo` | `VSCREEN-0069` | `origo` | `vento-origo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `origo` | `VSCREEN-0070` | `origo` | `vento-origo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `origo` | `VSCREEN-0071` | `origo` | `vento-origo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `origo` | `VSCREEN-0072` | `origo` | `vento-origo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `origo` | `VSCREEN-0073` | `origo` | `vento-origo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `origo` | `VSCREEN-0074` | `origo` | `vento-origo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONDITIONAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `origo` | `VSCREEN-0075` | `origo` | `vento-origo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `origo` | `VSCREEN-0076` | `origo` | `vento-origo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `origo` | `VSCREEN-0077` | `origo` | `vento-origo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `origo` | `VSCREEN-0078` | `origo` | `vento-origo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONDITIONAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `origo` | `VSCREEN-0079` | `origo` | `vento-origo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `pulso` | `VSCREEN-0080` | `pulso` | `vento-pulso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-ENTRY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `pulso` | `VSCREEN-0081` | `pulso` | `vento-pulso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `pulso` | `VSCREEN-0082` | `pulso` | `vento-pulso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `pulso` | `VSCREEN-0083` | `pulso` | `vento-pulso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `pulso` | `VSCREEN-0084` | `pulso` | `vento-pulso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `pulso` | `VSCREEN-0085` | `pulso` | `vento-pulso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `pulso` | `VSCREEN-0086` | `pulso` | `vento-pulso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `pulso` | `VSCREEN-0087` | `pulso` | `vento-pulso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `pulso` | `VSCREEN-0088` | `pulso` | `vento-pulso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `pulso` | `VSCREEN-0089` | `pulso` | `vento-pulso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `pulso` | `VSCREEN-0090` | `pulso` | `vento-pulso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONDITIONAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `pulso` | `VSCREEN-0091` | `pulso` | `vento-pulso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONDITIONAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `pulso` | `VSCREEN-0092` | `pulso` | `vento-pulso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `pulso` | `VSCREEN-0093` | `pulso` | `vento-pulso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONDITIONAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `numera` | `VSCREEN-0094` | `numera` | `vento-numera` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-ENTRY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `numera` | `VSCREEN-0095` | `numera` | `vento-numera` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `numera` | `VSCREEN-0096` | `numera` | `vento-numera` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `numera` | `VSCREEN-0097` | `numera` | `vento-numera` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONDITIONAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `numera` | `VSCREEN-0098` | `numera` | `vento-numera` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `numera` | `VSCREEN-0099` | `numera` | `vento-numera` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `numera` | `VSCREEN-0100` | `numera` | `vento-numera` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `numera` | `VSCREEN-0101` | `numera` | `vento-numera` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONDITIONAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `numera` | `VSCREEN-0102` | `numera` | `vento-numera` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONDITIONAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `numera` | `VSCREEN-0103` | `numera` | `vento-numera` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONDITIONAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `numera` | `VSCREEN-0104` | `numera` | `vento-numera` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `numera` | `VSCREEN-0105` | `numera` | `vento-numera` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONDITIONAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `numera` | `VSCREEN-0106` | `numera` | `vento-numera` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `pass` | `VSCREEN-0107` | `pass` | `vento-pass` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-ENTRY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `pass` | `VSCREEN-0108` | `pass` | `vento-pass` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONTEXTUAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `pass` | `VSCREEN-0109` | `pass` | `vento-pass` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `pass` | `VSCREEN-0110` | `pass` | `vento-pass` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONTEXTUAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `pass` | `VSCREEN-0111` | `pass` | `vento-pass` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `pass` | `VSCREEN-0112` | `pass` | `vento-pass` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `viso` | `VSCREEN-0113` | `viso` | `vento-viso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `viso` | `VSCREEN-0114` | `viso` | `vento-viso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `viso` | `VSCREEN-0115` | `viso` | `vento-viso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `viso` | `VSCREEN-0116` | `viso` | `vento-viso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONDITIONAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `viso` | `VSCREEN-0117` | `viso` | `vento-viso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONDITIONAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `viso` | `VSCREEN-0118` | `viso` | `vento-viso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `viso` | `VSCREEN-0119` | `viso` | `vento-viso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `viso` | `VSCREEN-0120` | `viso` | `vento-viso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONDITIONAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `viso` | `VSCREEN-0121` | `viso` | `vento-viso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONDITIONAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `viso` | `VSCREEN-0122` | `viso` | `vento-viso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONDITIONAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `viso` | `VSCREEN-0123` | `viso` | `vento-viso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `anima` | `VSCREEN-0124` | `anima` | `vento-anima` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `anima` | `VSCREEN-0125` | `anima` | `vento-anima` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `anima` | `VSCREEN-0126` | `anima` | `vento-anima` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `anima` | `VSCREEN-0127` | `anima` | `vento-anima` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `anima` | `VSCREEN-0128` | `anima` | `vento-anima` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `anima` | `VSCREEN-0129` | `anima` | `vento-anima` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONDITIONAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `anima` | `VSCREEN-0130` | `anima` | `vento-anima` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONDITIONAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `anima` | `VSCREEN-0131` | `anima` | `vento-anima` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `nexo` | `VSCREEN-0132` | `nexo` | `vento-nexo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `nexo` | `VSCREEN-0133` | `nexo` | `vento-nexo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `nexo` | `VSCREEN-0134` | `nexo` | `vento-nexo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONDITIONAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `nexo` | `VSCREEN-0135` | `nexo` | `vento-nexo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `nexo` | `VSCREEN-0136` | `nexo` | `vento-nexo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `nexo` | `VSCREEN-0137` | `nexo` | `vento-nexo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `nexo` | `VSCREEN-0138` | `nexo` | `vento-nexo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `nexo` | `VSCREEN-0139` | `nexo` | `vento-nexo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `nexo` | `VSCREEN-0140` | `nexo` | `vento-nexo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONDITIONAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `nexo` | `VSCREEN-0141` | `nexo` | `vento-nexo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `nexo` | `VSCREEN-0142` | `nexo` | `vento-nexo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `nexo` | `VSCREEN-0143` | `nexo` | `vento-nexo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `nexo` | `VSCREEN-0144` | `nexo` | `vento-nexo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `origo` | `VSCREEN-0145` | `origo` | `vento-origo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `origo` | `VSCREEN-0146` | `origo` | `vento-origo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONDITIONAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `pulso` | `VSCREEN-0147` | `pulso` | `vento-pulso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `pulso` | `VSCREEN-0148` | `pulso` | `vento-pulso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `pulso` | `VSCREEN-0149` | `pulso` | `vento-pulso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `pulso` | `VSCREEN-0150` | `pulso` | `vento-pulso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONDITIONAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `pulso` | `VSCREEN-0151` | `pulso` | `vento-pulso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `pulso` | `VSCREEN-0152` | `pulso` | `vento-pulso` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `numera` | `VSCREEN-0153` | `numera` | `vento-numera` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `numera` | `VSCREEN-0154` | `numera` | `vento-numera` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `numera` | `VSCREEN-0155` | `numera` | `vento-numera` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `numera` | `VSCREEN-0156` | `numera` | `vento-numera` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `numera` | `VSCREEN-0157` | `numera` | `vento-numera` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `numera` | `VSCREEN-0158` | `numera` | `vento-numera` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `numera` | `VSCREEN-0159` | `numera` | `vento-numera` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `pass` | `VSCREEN-0160` | `pass` | `vento-pass` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-ENTRY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `pass` | `VSCREEN-0161` | `pass` | `vento-pass` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `pass` | `VSCREEN-0162` | `pass` | `vento-pass` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `pass` | `VSCREEN-0163` | `pass` | `vento-pass` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `pass` | `VSCREEN-0164` | `pass` | `vento-pass` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONDITIONAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `pass` | `VSCREEN-0165` | `pass` | `vento-pass` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONTEXTUAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `pass` | `VSCREEN-0166` | `pass` | `vento-pass` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `pass` | `VSCREEN-0167` | `pass` | `vento-pass` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `pass` | `VSCREEN-0168` | `pass` | `vento-pass` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `pass` | `VSCREEN-0169` | `pass` | `vento-pass` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONDITIONAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `pass` | `VSCREEN-0170` | `pass` | `vento-pass` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-WORK-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `pass` | `VSCREEN-0171` | `pass` | `vento-pass` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `pass` | `VSCREEN-0172` | `pass` | `vento-pass` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `fogo` | `VSCREEN-0173` | `fogo` | `vento-fogo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONDITIONAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `fogo` | `VSCREEN-0174` | `fogo` | `vento-fogo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-CONDITIONAL-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `shell` | `VSCREEN-0175` | `shell` | `vento-shell` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `nexo` | `VSCREEN-0176` | `nexo` | `vento-nexo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |
| `nexo` | `VSCREEN-0177` | `nexo` | `vento-nexo` | `NOT_EXECUTED` | `NOT_EXECUTED` | `USAB-PROFILE-SECONDARY-001` | `NOT_EXECUTED` | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-058-USER-TEST-GATE-001`; `AUTH-UI-059` | `BLOQUEADA` | `NOT_EXECUTED` | `NOT_EXECUTED` | `DESTINO_POSTERIOR_REQUERIDO_ANTES_DE_RETIRAR` |

Reconciliación:

```text
EXPECTED_VSCREEN = 177
MATERIALIZED_VSCREEN = 177
MISSING = 0
DUPLICATES = 0

RESULT_APROBADA = 0
RESULT_BLOQUEADA = 177
RESULT_NO_APLICA = 0
```

La distribución por aplicación se conserva:
...
#### 7. Gate `AUTH-UI-060-FINAL-SCREEN-APPROVAL-GATE-001`

Estado inicial:

```text
GATE_ID = AUTH-UI-060-FINAL-SCREEN-APPROVAL-GATE-001
STATUS = PENDIENTE_DE_EVIDENCIA
CURRENT_APPROVED = 0
CURRENT_BLOCKED = 177
CURRENT_NO_APLICA = 0
...
| Condición | Regla |
| --- | --- |
| `C01` | las 177 identidades aparecen exactamente una vez |
| `C02` | `AUTH-UI-058-USER-TEST-GATE-001 = PASS_TO_AUTH_UI_059` |
| `C03` | el gate de reconciliación de 059 permite consumo por 060 |
| `C04` | cada superficie tiene propietario funcional y técnico identificados |
| `C05` | cada superficie aplicable conserva actores y dispositivos realmente probados |
| `C06` | cada superficie aplicable conserva criterios y sesiones de evidencia reales |
| `C07` | no existen problemas `CRITICAL` o `HIGH` abiertos incompatibles con aprobación |
| `C08` | autorización, accesibilidad y recuperación cumplen la evidencia exigida |
| `C09` | ninguna superficie depende de una implementación pendiente si pretende `APROBADA` |
| `C10` | toda superficie `BLOQUEADA` tiene tarea o package posterior concreto que impide retirarla del roadmap |
| `C11` | cada `NO_APLICA` conserva justificación revisable |
| `C12` | cada decisión conserva revisor y fecha reales |

Resultados del gate:

```text
PASS_AND_HANDOFF_TO_OPS_REC_001
FAIL
PENDIENTE_DE_EVIDENCIA
```

`PASS_AND_HANDOFF_TO_OPS_REC_001` solo significa que las 177 identidades quedaron decididas de forma íntegra y trazable. No autoriza implementación física ni borra tareas/packages que continúen abiertos.

---

#### 8. Regla de aprobación individual

Una superficie solo puede cambiar de `BLOQUEADA` a `APROBADA` cuando:

1. tiene evidencia real de los actores y dispositivos aplicables;
2. consume los criterios de 057;
3. las sesiones de 058 son trazables;
4. la reconciliación 059 está completa para esa identidad;
5. no conserva problemas `CRITICAL` o `HIGH` abiertos incompatibles;
6. autorización, accesibilidad y recuperación son conformes;
7. toda implementación requerida está disponible;
8. tiene revisor real;
9. tiene fecha real;
10. no existe bloqueador abierto que invalide la decisión.

---

#### 9. Regla de bloqueo

`BLOQUEADA` se conserva cuando existe cualquiera de estas condiciones:

```text
EVIDENCIA AUSENTE
GATE 058 PENDIENTE
RECONCILIACION 059 PENDIENTE
CRITICAL/HIGH ABIERTO INCOMPATIBLE
AUTORIZACION NO CONFORME
ACCESIBILIDAD NO CONFORME
RECUPERACION NO CONFORME
IMPLEMENTACION NECESARIA PENDIENTE
REVISOR AUSENTE
FECHA AUSENTE
DESTINO POSTERIOR NO RESUELTO
```

Una superficie bloqueada no puede retirarse del roadmap.

Antes del PASS global, cada bloqueo deberá quedar vinculado a una tarea o package posterior concreto que preserve el trabajo pendiente.

---

#### 10. Regla `NO_APLICA`

`NO_APLICA` se permite únicamente cuando:

- la superficie realmente no participa del alcance evaluado;
- existe justificación revisable;
- la justificación no oculta ausencia de implementación, falta de evidencia o falta de participantes;
- la decisión tiene revisor y fecha;
- el resultado no elimina una obligación vigente.

---

#### 11. Revisor y fecha

El gate real debe registrar:
...
Mientras la revisión no se ejecute:

```text
reviewer = NOT_EXECUTED
decision_date = NOT_EXECUTED
```

Estos valores son evidencia explícita de no ejecución y no equivalen a placeholders de aprobación.

---

#### 12. Handoff al siguiente tramo normal

Después del PASS global:

```text
AUTH-UI-060-FINAL-SCREEN-APPROVAL-GATE-001
=
PASS_AND_HANDOFF_TO_OPS_REC_001
```

la continuidad documental normal pasa a:

```text
OPS-REC-001 — Definir el contrato canónico de recetas y acceso contextual
```

Las superficies `BLOQUEADA` no se consideran retiradas del roadmap; sus tareas/packages posteriores permanecen vigentes hasta resolverlas.

---

#### 13. Estado actual

| Materia | Estado actual | Propietario | Condición de salida |
| --- | --- | --- | --- |
| contrato de decisión 060 | `ESPECIFICADO` | `AUTH-UI-060` | aprobación documental |
| matriz de 177 identidades | `MATERIALIZADA` | `AUTH-UI-060` | evaluación real posterior |
| evidencia de usuarios | `PENDIENTE_DE_EVIDENCIA` | gate 058 | sesiones suficientes |
| reconciliación de problemas | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-059` | gate 059 habilita 060 |
| revisor/fecha | `NOT_EXECUTED` | gate 060 | revisión real |
| destinos posteriores de bloqueos | `PENDIENTE_DE_EVIDENCIA` | owner técnico/funcional correspondiente | tarea/package concreto por bloqueo |
| gate 060 | `PENDIENTE_DE_EVIDENCIA` | `AUTH-UI-060-FINAL-SCREEN-APPROVAL-GATE-001` | C01..C12 satisfechas |
...
Justificación: esta tarea define una puerta documental de decisión y reconciliación sobre superficies, criterios y evidencia ya existentes. No crea comportamiento ejecutable, autorización, navegación, dato persistente, integración, ruta, pantalla, proceso ni efecto empresarial nuevo; tampoco modifica, difiere, descarta o declara obsoleto ningún requisito histórico. El registro canónico de requisitos de prueba no cambia.

---

#### 15. Cobertura de prueba vigente reutilizada

Se reutilizan los criterios de 057, el método y evidencia de 058, la reconciliación de 059 y las pruebas propietarias de cualquier implementación o corrección vinculada.
...
| Clase | Estado | Evidencia |
| --- | --- | --- |
| BUILD | NOT_EXECUTED | La compilación documental real corresponde al checkout local después de incorporar 059 y reemplazar este artefacto. |
| LOCAL | NOT_EXECUTED | Formato, quality, delivery, BLOQUE I, topología y batería global quedan pendientes del checkout local. |
| REMOTA | PASS | Se verificaron título, owner, condición histórica de cierre, ruta normal, handoff vigente a `OPS-REC-001`, ruta diferida separada de `AUTH-UI-061`, topología, catálogo de aplicaciones, `package.json`, preflight y validadores aplicables; 059 se consume desde su versión completa aprobada mientras su incorporación permanece pendiente. |
| OPERATIVA | NOT_EXECUTED | No se ejecutó el gate final; las 177 superficies permanecen `BLOQUEADA` por evidencia/revisión pendientes. |
| FÍSICA | NOT_APPLICABLE | La tarea no crea instancia física propia ni modifica producto o infraestructura. |
...
- [ ] las 177 identidades aparecen exactamente una vez;
- [ ] existen cero faltantes y cero duplicados;
- [ ] cada fila contiene aplicación, propietario funcional y propietario técnico;
- [ ] cada fila conserva actores, dispositivos, criterios, sesiones, problemas, bloqueadores, resultado, revisor y fecha;
- [ ] los únicos resultados son `APROBADA`, `BLOQUEADA` y `NO_APLICA`;
- [ ] ninguna superficie sin evidencia real aparece `APROBADA`;
- [ ] ninguna superficie con `CRITICAL` o `HIGH` incompatible abierto aparece `APROBADA`;
- [ ] ninguna superficie dependiente de implementación pendiente aparece `APROBADA`;
- [ ] cada `NO_APLICA` exige justificación revisable;
- [ ] toda superficie bloqueada conserva obligación de destino posterior antes del PASS global;
- [ ] la matriz inicial refleja 0 aprobadas, 177 bloqueadas y 0 no aplica;
- [ ] revisor y fecha permanecen `NOT_EXECUTED` hasta revisión real;
- [ ] el gate 060 inicia `PENDIENTE_DE_EVIDENCIA`;
- [ ] `AUTH-UI-061` no se usa como continuidad normal del minibloque;
- [ ] la continuidad normal queda en `OPS-REC-001`;
- [ ] no se crean o modifican requisitos de prueba;
- [ ] no se ejecutan cambios físicos.
...
Este contrato no aprueba realmente ninguna superficie. La decisión actual segura es `BLOQUEADA` para las 177 identidades hasta que existan evidencia, reconciliación, destinos de bloqueo, revisor y fecha suficientes.
