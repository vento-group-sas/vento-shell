...
export function renderManualDocumentationAhead({ task, workTopology }) {
  ...
  return `TRABAJO DOCUMENTAL ADELANTADO EN CHATGPT — MODO MANUAL

MANUAL_DOCUMENTATION_AHEAD: ENABLED
MAX_UNPUBLISHED_PREDECESSORS: 1
REMOTE_PUBLICATION_REQUIRED_FOR_DRAFT: FALSE
REPOSITORY_WRITE_BEFORE_PREDECESSOR_CLOSE: FALSE

Cuando el usuario solicite la siguiente tarea mientras termina el comando de la actual, redacta y entrega el archivo descargable completo de la sucesora. No respondas que debes esperar a que la anterior aparezca en GitHub, termine su PR o publique NEXT_TASK_ALLOWED: SI para poder redactar.

El ID actual de este iniciador es una fotografía de su generación, no un bloqueo permanente de la conversación. Resuelve la sucesora por la ruta canónica y su marcador exacto; nunca sumes uno al número del ID. La solicitud de adelantar documentación no autoriza cambiar marcadores ni iniciar otra rama.

CONTEXTO PARA ADELANTAR

- Usa la última versión completa aprobada por el usuario de la tarea anterior, incluida la entregada en este chat o adjunta como archivo aunque el remoto aún tenga el marcador antiguo.
- Usa igualmente el 04A resultante de esa entrega si cambió TREQ. No asignes IDs ni redactes contra una versión anterior del registro.
- Conserva nombre y contenido exactos de los archivos usados como base; si dispones del SHA-256 calculado, consérvalo. No inventes hashes ni resultados de publicación.
- Si falta el contenido necesario, pide únicamente ese archivo o fragmento completo. La ausencia del commit remoto no es por sí sola una carencia de contenido.
- Las demás dependencias se leen de sus fuentes canónicas. Una contradicción material se resuelve antes de incorporar la entrega, no mediante una aprobación supuesta.
- Para la sucesora consulta su propio archivo propietario, topología y dependencias; los datos de la tarea inicial son una referencia y no sustituyen los de la nueva tarea solicitada.

ENTREGA Y COMANDO HABITUAL
...
- No adelantes una segunda tarea sobre otra todavía no incorporada. No ejecutes cambios físicos ni modifiques active-sequence.json para simular avance.

SUCESORA DOCUMENTAL EN LA RUTA CONSULTADA

${next ? `- ID: ${next.id}
- Título: ${next.title}
- Archivo propietario: docs/plan-canonico/modular/${next.relativePath}
- Uso: referencia para la solicitud explícita del usuario; no constituye inicio ni aprobación.

MARCADOR Y CONTENIDO CANÓNICO DE REFERENCIA DE LA SUCESORA

${next.block.trim()}` : 'No hay sucesora pendiente identificada en esta ruta. No inventes una tarea ni saltes a implementación.'}
`;
}
